...
def run_SVM_traintest(shading=True,num_iterations=100):
    ...
    report_all=load_object_from_file("Grid-search_report_SVM.pk1",
                                         to_file="SVM",shading=shading)
    ...
    train_time_GS=get_GS_traintime(to_file="SVM", shading=shading)
    ...
    params=support_vm.get_params() #get params of model
    write_output_to_csv(report_all.round(4),cm_all,params,file_name="Test-train-aggregated-results_SVM",
                        to_file="SVM",shading=shading)
      
    #plot Confusion Matrix and save to pdf
    plot_confusion_matrix(cm_all,to_file="SVM",show_plot=False,normalize=True,
                          shading=shading,
                          title=f"TestTrain ConfusionMatrix SVM shading {shading}")
