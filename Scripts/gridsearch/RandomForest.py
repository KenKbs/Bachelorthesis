# -*- coding: utf-8 -*-
"""
Created on Mon Mar 11 10:44:10 2024

@author: Kenny
"""

# -*- coding: utf-8 -*-
"""
Created on Mon Feb 12 15:21:50 2024

@author: Kenny
"""
#%% Imports 
#Main package imports done in util.py

#Internal imports
from Scripts.util import (
    get_data,
    filter_data,
    generate_table,
    train_test_split_data,
    perform_grid_search,
    get_performance_metrics,
    get_confusion_matrix,
    plot_confusion_matrix,
    write_output_to_csv,
    save_object_to_file,
    load_object_from_file,#REMOVE LATER!
    get_filepath
)

#SK-learn model imports
from sklearn.ensemble import (RandomForestClassifier
                                                    )
from sklearn.tree import (export_graphviz,
                          plot_tree
                                    )


# #Other Imports
# import pandas as pd
import numpy as np
import matplotlib.pyplot as plt

#try importing graphviz problems, if not correctly installed
# command with conda: conda install -c conda-forge pygraphviz
try:   
    import graphviz
except Exception as e:
    print("Failed to import module graphviz",e,"\n")
    print("Using Matplotlib")






#%% Gridsearch
def run_RF_gridsearch(shading=True):
    """
    Runs Gridsearch for decision Tree and writes the best results
    to directory Results/DT

    Parameters
    ----------
    shading : Bool, optional
        if True, includes shading class
        Else excludes it. The default is True.

    Returns
    -------
    None.

    """
    pass
#put later into function
shading=True

#Read-in Data
raw_data=get_data()

#filter data
data=filter_data(raw_data,filter_value=100,shading=shading)

#Print out fault distribution before and after filtering

#Include shading cases
if shading:
    generate_table(raw_data,data,"Raw","Filtered")

#Exclude shading casees
else:
    generate_table(raw_data,data,"Raw","Shad. excl")
            

# Grid search preparations

# Split data w. own fuinction, scaling = False
x_train, x_test, y_train, y_test = train_test_split_data(data=data,
                                                         test_size=0.2,
                                                         scaling=False)

#%% Define Model to tune (Decision Tree)
rForest=RandomForestClassifier(class_weight=None,
                              min_weight_fraction_leaf=0.0,
                              min_impurity_decrease=0.0,
                              max_leaf_nodes=None,
                              min_samples_leaf=1)

#%% Set Parameter to tune 
"""

# Criterion
Criterion=["gini","entropy"] #,"log_loss"] #log loss same as shanon entropy! #Gini usually better!
    
#max_depth 
max_depth=[None] #- prevent overfitting, depth of tree
#Define range to search for (taken from overfitted_tree for both cases)
max_range=41 if shading else 21 
max_depth.extend(x for x in range (1,max_range,1))
    
#max_leaf_nodes
# max_leaf_nodes=[None] # maximum number of leaf nodes    
#append values from 50 to 3050 in 50 steps
# max_leaf_nodes.extend(x for x in range (500,3001,100))
    
#min_samples_split  
min_samples_split=[2] #minimum number of samples requiered for splitting a node    
#append values from   
min_samples_split.extend(x for x in range(3,26,1))
   
#min_samples_leaf 
# min_samples_leaf=[1] # minimum number of samples required in last decision leafs
# min_samples_leaf.extend(x for x in range(2,26,1))
    
#max_features
max_features=[None] # - randomly select x features, from these x features, determine which is best to split and do the splitting    
#append 1 to 5, because 6 = maxfeatures, this case covered with None
max_features.extend(x for x in range (1,6,1))    
    
#ccp_alpha 
ccp_alpha=[0.0] ## minimal cost pruning, after fitting complete prune tree - small alpha values!!!    
#append 0.005 to 0.501 in 0.005 steps
ccp_alpha.extend(np.arange(0.005,0.501,0.005))

###ADDITIONAL RF VARIABLES######
bootstrap=[True] ##look into that, parameters like max_samples depend on that!
bootstrap.extend(False)

n_estimators=[100] #default
n_estimators.extend(x for x in range (1,100,5))
#njobs????
#verbose????
#random_state????
#max_samples??? int,float or None (default)

    
#Give over parameters to parameter grid to search for
param_grid={'criterion':Criterion,
            'max_depth':max_depth,
            'min_samples_split':min_samples_split,
            # 'min_samples_leaf':min_samples_leaf,
            'max_features':max_features,
            # 'max_leaf_nodes':max_leaf_nodes,
            'ccp_alpha':ccp_alpha,
            'bootstrap':bootstrap
            }

#TESTING
param_grid_t={'criterion':Criterion} #REMOVE LATER AND CHANGE FUNCTION CALL!

#%%Perform Grid_search
best_model,cv_results=perform_grid_search(x_train,y_train,rForest,param_grid_t)

#Get best_model parameters
best_model_params=best_model.get_params()


#%%Performance evaluation

#Predictions on test-set
y_pred_best=best_model.predict(x_test)

#Get Accuracy Score
accuracy=get_performance_metrics(y_test, y_pred_best,only_accuracy=True)

#Get f1,recall,precision etc.
report=get_performance_metrics(y_test, y_pred_best) #report=df

#Get confusion Matrix
cm=get_confusion_matrix(y_test, y_pred_best,normalize=False)
    

#%% Save Results to file / csv 

#plot Confusion Matrix and save to file
plot_confusion_matrix(cm,to_file="RF",show_plot=False,normalize=True,
                      shading=shading,
                      title=f"Grid-search ConfusionMatrix RF shading {shading}")
    
#save best model to file
save_object_to_file(best_model,file_name="Best_Model",
                    to_file="RF",shading=shading)

#save confusion matrix to file:
save_object_to_file(cm,file_name="Grid-search_CM",
                    to_file="RF",shading=shading)

#save report (f1_score etc.) to file:
save_object_to_file(report,file_name="Grid-search_report",
                    to_file="RF",shading=shading)

#Save Grid-search results to csv_file
write_output_to_csv(cv_results,output2=report.round(4), #take rounded numbers of report for better overview
                    output3=best_model_params,
                    file_name="Grid-search-results_RF",
                    to_file="RF",shading=shading)
"""



#Get output path
parent_file_path=get_filepath(model_sd="RF",shading=shading)
file_path=parent_file_path+r'\Gridsearch_RF'

#%% Plot and save decision Tree

best_model=load_object_from_file("Best_Model_RF.pk1",
                                 to_file="RF",shading=True) #TESTING!

#Extract feature and class names
fn=x_test.columns.tolist()
cn=y_test.unique().tolist()
cn=sorted(cn)
#cast into string
cn=[str(Float) for Float in cn]


# fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(30,30),dpi=800)
# plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
#                filled=True,fontsize=6)

# fig.savefig('rf_individualtree1.png')
# fig.savefig('rf_individualtree1.pdf')



try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(10,5),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=6)
    fig.savefig('rf_individualtree1.png')
    fig.savefig('rf_individualtree1.pdf')
except Exception as e:
    print ("Määäp, Too large - number 1",e)

try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(30,15),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=6)
    fig.savefig('rf_individualtree2.png')
    fig.savefig('rf_individualtree2.pdf')
except Exception as e:
    print ("Määäp, Too large - number 2",e)

try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(60,30),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=6)
    fig.savefig('rf_individualtree3.png')
    fig.savefig('rf_individualtree3.pdf')
except Exception as e:
    print ("Määäp, Too large - number 3",e)

try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(120,60),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=6)
    fig.savefig('rf_individualtree4.png')
    fig.savefig('rf_individualtree4.pdf')
except Exception as e:
    print ("Määäp, Too large - number 4",e)

try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(240,120),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=6)
    fig.savefig('rf_individualtree5.png')
    fig.savefig('rf_individualtree5.pdf')
except Exception as e:
    print ("Määäp, Too large - number 5",e)
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(480,240),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=6)
    fig.savefig('rf_individualtree6.png')
    fig.savefig('rf_individualtree6.pdf')
except Exception as e:
    print ("Määäp, Too large - number 6",e)
    
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(20,5),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=6)
    fig.savefig('rf_individualtree7.png')
    fig.savefig('rf_individualtree7.pdf')
except Exception as e:
    print ("Määäp, Too large - number 7",e)
    
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(40,10),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=6)
    fig.savefig('rf_individualtree8.png')
    fig.savefig('rf_individualtree8.pdf')
except Exception as e:
    print ("Määäp, Too large - number 8",e)

try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(80,20),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=6)
    fig.savefig('rf_individualtree9.png')
    fig.savefig('rf_individualtree9.pdf')
except Exception as e:
    print ("Määäp, Too large - number 9",e)
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(160,40),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=6)
    fig.savefig('rf_individualtree10.png')
    fig.savefig('rf_individualtree10.pdf')
except Exception as e:
    print ("Määäp, Too large - number 10",e)
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(200,50),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=6)
    fig.savefig('rf_individualtree11.png')
    fig.savefig('rf_individualtree11.pdf')
except Exception as e:
    print ("Määäp, Too large - number 11",e)
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(500,125),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=6)
    fig.savefig('rf_individualtree12.png')
    fig.savefig('rf_individualtree12.pdf')
except Exception as e:
    print ("Määäp, Too large - number 12",e)
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(100,50),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=6)
    fig.savefig('rf_individualtree13.png')
    fig.savefig('rf_individualtree13.pdf')
except Exception as e:
    print ("Määäp, Too large - number 13",e)
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(90,40),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=6)
    fig.savefig('rf_individualtree14.png')
    fig.savefig('rf_individualtree14.pdf')
except Exception as e:
    print ("Määäp, Too large - number 14",e)

try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(80,30),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=6)
    fig.savefig('rf_individualtree15.png')
    fig.savefig('rf_individualtree15.pdf')
except Exception as e:
    print ("Määäp, Too large - number 15",e)
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(70,20),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=6)
    fig.savefig('rf_individualtree16.png')
    fig.savefig('rf_individualtree16.pdf')
except Exception as e:
    print ("Määäp, Too large - number 16",e)
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(60,10),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=6)
    fig.savefig('rf_individualtree17.png')
    fig.savefig('rf_individualtree17.pdf')
except Exception as e:
    print ("Määäp, Too large - number 17",e)
    
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(50,10),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=6)
    fig.savefig('rf_individualtree18.png')
    fig.savefig('rf_individualtree18.pdf')
except Exception as e:
    print ("Määäp, Too large - number 18",e)
    
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(50,50),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=6)
    fig.savefig('rf_individualtree19.png')
    fig.savefig('rf_individualtree19.pdf')
except Exception as e:
    print ("Määäp, Too large - number 19",e)
    
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(60,50),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=6)
    fig.savefig('rf_individualtree20.png')
    fig.savefig('rf_individualtree20.pdf')
except Exception as e:
    print ("Määäp, Too large - number 20",e)
    
    
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(70,50),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=6)
    fig.savefig('rf_individualtree21.png')
    fig.savefig('rf_individualtree21.pdf')
except Exception as e:
    print ("Määäp, Too large - number 21",e)
    
    
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(80,50),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=6)
    fig.savefig('rf_individualtree22.png')
    fig.savefig('rf_individualtree22.pdf')
except Exception as e:
    print ("Määäp, Too large - number 22",e)
    
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(90,50),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=6)
    fig.savefig('rf_individualtree23.png')
    fig.savefig('rf_individualtree23.pdf')
except Exception as e:
    print ("Määäp, Too large - number 23",e)
    
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(80,40),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=6)
    fig.savefig('rf_individualtree24.png')
    fig.savefig('rf_individualtree24.pdf')
except Exception as e:
    print ("Määäp, Too large - number 24",e)
    

try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(200,60),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=4)
    fig.savefig('rf_individualtree25.png')
    fig.savefig('rf_individualtree25.pdf')
except Exception as e:
    print ("Määäp, Too large - number 25",e)
    

try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(230,70),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=4)
    fig.savefig('rf_individualtree26.png')
    fig.savefig('rf_individualtree26.pdf')
except Exception as e:
    print ("Määäp, Too large - number 26",e)
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(240,80),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=4)
    fig.savefig('rf_individualtree27.png')
    fig.savefig('rf_individualtree27.pdf')
except Exception as e:
    print ("Määäp, Too large - number 27",e)
    
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(250,90),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=4)
    fig.savefig('rf_individualtree28.png')
    fig.savefig('rf_individualtree28.pdf')
except Exception as e:
    print ("Määäp, Too large - number 28",e)
    
    
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(300,100),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=4)
    fig.savefig('rf_individualtree29.png')
    fig.savefig('rf_individualtree29.pdf')
except Exception as e:
    print ("Määäp, Too large - number 29",e)
    
    
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(350,60),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=4)
    fig.savefig('rf_individualtree30.png')
    fig.savefig('rf_individualtree30.pdf')
except Exception as e:
    print ("Määäp, Too large - number 30",e)
    
    
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(400,100),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=4)
    fig.savefig('rf_individualtree31.png')
    fig.savefig('rf_individualtree31.pdf')
except Exception as e:
    print ("Määäp, Too large - number 31",e)
    
    
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(250,50),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=4)
    fig.savefig('rf_individualtree32.png')
    fig.savefig('rf_individualtree32.pdf')
except Exception as e:
    print ("Määäp, Too large - number 32",e)
    
    
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(300,90),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=4)
    fig.savefig('rf_individualtree33.png')
    fig.savefig('rf_individualtree33.pdf')
except Exception as e:
    print ("Määäp, Too large - number 33",e)
    
    
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(400,50),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=4)
    fig.savefig('rf_individualtree34.png')
    fig.savefig('rf_individualtree34.pdf')
except Exception as e:
    print ("Määäp, Too large - number 34",e)
    
try:
    fig,axes = plt.subplots(nrows=1,ncols=1,figsize=(500,90),dpi=100)
    plot_tree(best_model.estimators_[0],feature_names=fn, class_names=cn,
                   filled=True,fontsize=4)
    fig.savefig('rf_individualtree35.png')
    fig.savefig('rf_individualtree35.pdf')
except Exception as e:
    print ("Määäp, Too large - number 35",e)




# With Graphviz OLD!!!
# try: 
#     dot_data = export_graphviz(best_model, out_file=None, filled=True, rounded=True, special_characters=True)
#     graph = graphviz.Source(dot_data)
#     graph.render(file_path, format="pdf")  # Save the visualization as a file
#     graph.view()  # Display the decision tree in the default viewer

# #if Error, render with matplotlib
# except Exception as e:
#     print("Error occured while rendering with graphviz:",e,"\n\n")
#     print("Attempt to render using Matplotlib")           
#     file_path+=r'.pdf'
#     plt.figure(figsize=(120, 60))  # Set the figure size
#     plot_tree(best_model, filled=True, rounded=True)#, feature_names=feature_names, class_names=target_names)  # Plot the decision tree
#     plt.savefig(file_path)  # Save the plot to a file

"""
graphviz does not work from console, if not added to system path
because adding to system path is kinda inconvenient, did a try except statement
with a different visualisation tool.
"""

#%%Overfit Decision Tree (just default values)
def overfit_RF (shading=True,runs=100):
    pass
    # #%% Read in and filter data
    
    # #Read-in Data
    # raw_data=get_data()
    
    # #filter data
    # data=filter_data(raw_data,filter_value=100,shading=shading)
    
    # #Print out fault distribution before and after filtering
    
    # #Include shading cases
    # if shading:
    #     generate_table(raw_data,data,"Raw","Filtered")
    
    # #Exclude shading casees
    # else:
    #     generate_table(raw_data,data,"Raw","Shad. excl")
                        
    # #Starting values
    # #define empty lists
    # num_nodes_all=[]
    # tree_depth_all=[]
    # num_features_all=[]
    # accuracy_all=[]   
    
    # #number of runs with "overfitted" Tree to see stats of that algorithm
    # for run in range (runs):
    #     # Split data w. own fuinction, scaling = False
    #     x_train, x_test, y_train, y_test = train_test_split_data(data=data,
    #                                                              test_size=0.2,
    #                                                              scaling=False)
        
    #     #Define Model to tune (Logistic Regression)
    #     dctree=DecisionTreeClassifier(splitter="best",criterion="gini",
    #                                   max_depth=None,min_samples_split=2,
    #                                   min_samples_leaf=1,max_features=None,
    #                                   ccp_alpha=0.0,max_leaf_nodes=None,
    #                                   min_impurity_decrease=0.0)
        
    #     #%% TESTING OF DECISION TREE to determine which paramter ranges to tune!
        
    #     best_model=dctree
    #     best_model.fit(x_train,y_train)
           
    #     #Predictions on test-set
    #     y_pred_best=best_model.predict(x_test)
                            
    #     #Print out stuff
    #     #current run    
    #     print(f"Current run: {run+1} \n")
        
    #     #Number nodes
    #     num_nodes = best_model.tree_.node_count
    #     print("Number of nodes:", num_nodes,"\n")
    #     num_nodes_all.append(num_nodes)
        
    #     #depth
    #     tree_depth = best_model.get_depth()
    #     print("Depth of the tree:", tree_depth,"\n")
    #     tree_depth_all.append(tree_depth)
        
    #     #Features (should be the same for every run)
    #     num_features = best_model.n_features_in_
    #     print("Number of features:", num_features,"\n")
    #     num_features_all.append(num_features)
        
    #     #Number of classes
    #     num_classes = best_model.n_classes_
    #     print("Number of classes:", num_classes,"\n")
        
    #     #Feature importance - will not be used later, would imply class_weights
    #     feature_importance = best_model.feature_importances_
    #     print("Feature importance:", feature_importance,"\n")
            
    #     #Get Accuracy Score
    #     accuracy=get_performance_metrics(y_test, y_pred_best,only_accuracy=True)
    #     accuracy_all.append(accuracy)
        
    #     #Print seperator
    #     print ("*"*40)
    
    # #%% Save Results to file / csv 
    
    # #Get confusion Matrix only for last run as example
    # cm=get_confusion_matrix(y_test, y_pred_best,normalize=False)
    
    # #plot last Confusion Matrix and save to file
    # plot_confusion_matrix(cm,to_file="DT",show_plot=False,normalize=True,
    #                       shading=shading,
    #                       title=f"Overfitted ConfusionMatrix DT shading {shading}")
    
    # #create Dataframe and write to csv
    # dt_overfitted=pd.DataFrame()
    
    # #Append important values
    # dt_overfitted["Number of nodes:"]=num_nodes_all
    # dt_overfitted["Depth of the tree:"]=tree_depth_all
    # dt_overfitted["Number of features:"]=num_features_all
    # dt_overfitted["Accuracy"]=accuracy_all
    
    # #Append Max-Values
    # max_values = dt_overfitted.max()
    # dt_overfitted.loc[len(dt_overfitted)] = max_values
    
    # #Re-adjust index
    # dt_overfitted.rename({dt_overfitted.index[-1]:"Max"},inplace=True)
    
    # #Get model Params:
    # model_params=best_model.get_params()
    
    # #write Dataframe to csv
    # write_output_to_csv(dt_overfitted,output3=model_params,
    #                     file_name="Overfitted_tree",
    #                     to_file="DT",shading=shading)
    # #Get output path
    # parent_file_path=get_filepath(model_sd="DT",shading=shading)
    # file_path=parent_file_path+r'\Overfitted_tree'
    
    # # Plot and save decision Tree only of last run as example
    # # With Graphviz
    # try: 
    #     dot_data = export_graphviz(best_model, out_file=None, filled=True, rounded=True, special_characters=True)
    #     graph = graphviz.Source(dot_data)
    #     graph.render(file_path, format="pdf")  # Save the visualization as a file
    #     graph.view()  # Display the decision tree in the default viewer    
    
    # #With matplotlib if it fails
    # except Exception as e:
    #     print("Error occured while rendering with graphviz:",e,"\n\n")
    #     print("Attempt to render using Matplotlib")       
    #     file_path+=r'.pdf'
    #     plt.figure(figsize=(120, 60))  # Set the figure size
    #     plot_tree(best_model, filled=True, rounded=True)#, feature_names=feature_names, class_names=target_names)  # Plot the decision tree
    #     plt.savefig(file_path)  # Save the plot to a file
    
    
#run function for testing:

